...
erDiagram
    crime_codes {
        VARCHAR(10) iucr PK "Código IUCR"
        VARCHAR(200) primary_type
        VARCHAR(200) description
        VARCHAR(10) fbi_code
    }
    blocks {
        BIGSERIAL id PK
        VARCHAR(200) block "Nombre de la calle o bloque"
    }
    locations_descriptions {
        BIGSERIAL id PK
        VARCHAR(200) location_description
    }
    coordinates {
        BIGSERIAL id PK
        BIGINT x_coordinate
        ...
        VARCHAR(200) location "Etiqueta textual"
    }
    locations {
        BIGSERIAL id PK
        BIGINT block_id FK "→ blocks.id"
        BIGINT description_id FK "→ locations_descriptions.id"
        BIGINT coordinate_id FK "→ coordinates.id"
    }
    crimes {
        BIGSERIAL id PK
        VARCHAR(200) case_number "Número de caso"
        TIMESTAMP crime_date
        VARCHAR(10) iucr FK "→ crime_codes.iucr"
